...
class ImageEncoder(torch.nn.Module):
    def __init__(self, args, keep_lang=False):
        ...
        # self.train_preprocess.transforms[-1].mean
        pixel_mean = torch.tensor(self.train_preprocess.transforms[-1].mean).to(args.device).view(3,1,1)
        pixel_std = torch.tensor(self.train_preprocess.transforms[-1].std).to(args.device).view(3,1,1)
        self.post_normalize = args.post_normalize
...
class ClassificationHead(torch.nn.Linear):
    ...
    def forward(self, inputs):
        ...
        return super().forward(inputs.to(dtype=dtype))
