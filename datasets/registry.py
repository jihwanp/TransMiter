...
from datasets.few_shot import FewshotDataset
...
def get_dataset(dataset_name, preprocess, location, batch_size=128, num_workers=16, val_fraction=0.1, max_val_samples=5000,
                is_transfer=False, use_train_for_proxy=False, data_ratio=1.0,args=None,dg_type=None, post_train=False):
    if args.setting is not None:
        dataset = FewshotDataset(
            preprocess, location=location, batch_size=batch_size, num_workers=args.num_workers, is_transfer=is_transfer, use_train_for_proxy=use_train_for_proxy, data_ratio=data_ratio, args=args,dg_type=dg_type,
    ...
    if dataset_name.endswith('Val'):
        ...
        if dataset_name in registry:
            ...
            base_dataset = get_dataset(base_dataset_name, preprocess, location, batch_size, args.num_workers)
            dataset = split_train_into_train_val(
                base_dataset, dataset_name, batch_size, args.num_workers, val_fraction, max_val_samples)
            return dataset
    else:
        assert dataset_name in registry, f'Unsupported dataset: {dataset_name}. Supported datasets: {list(registry.keys())}'
        dataset_class = registry[dataset_name]
    dataset = dataset_class(
        preprocess, location=location, batch_size=batch_size, num_workers=args.num_workers, is_transfer=is_transfer, use_train_for_proxy=use_train_for_proxy, data_ratio=data_ratio, args=args
    )
    return dataset
