...
def maybe_dictionarize(batch,procrutes_mode=False):
    if isinstance(batch, dict):
        if batch['labels'].dim()==2:
            if procrutes_mode:
                # random_idx = -1
                # batch = {k:v[:,random_idx] for k,v in batch.items()}
                batch = {k:v.flatten(0,1) for k,v in batch.items()}
            else:
                random_idx = torch.randint(0, batch['labels'].size(1), (batch['labels'].size(0),1))
                # mask = 
                # random_idx = -1
                batch = {k:torch.gather(v,1,random_idx) if k=='labels' else \
                    torch.gather(v,1,random_idx.unsqueeze(-1).repeat(1,1,v.shape[-1])) for k,v in batch.items()}
...
def get_dataloader(dataset, is_train, args, image_encoder=None, is_transfer=False):
    if image_encoder is not None:
        ...
        if args.use_precompute_features:
            if is_transfer and args.loader_ver2:
                return dataset.transfer_feats_loader
            else:
                return dataset.train_feats_loader if is_train else dataset.test_feats_loader
        
        if is_transfer:
            dataloader = dataset.proxy_loader_shuffle
        else:
